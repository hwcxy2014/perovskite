...
for iter in range(nIter):
	# get next batch by EI
	next_sample = gp.getNextSample(batchSize)
	print '--------------',next_sample
	# convert to simulator readable format
	next_f = [[] for i in range(batchSize)]
	for x in range(batchSize):	
		next_f[x] = read_numbering_solutions.get_solution(\
						next_sample[x]+1)
	# evaluate the solubilities of comp in next batch
	solu = Parallel(n_jobs=batchSize)\
			(delayed(get_UMBO)([next_f[i][0],\
			next_f[i][1],next_f[i][2]],next_f[i][3],\
			next_f[i][4]) for i in range(batchSize))
	BOList[iter] = solu + next_sample
	# update the output file
	with open(r'BO_result.csv','a') as f:
		writer = csv.writer(f)
		writer.writerow(solu + next_sample) 
	# update gp
	for x in range(batchSize):
		gp.updatePosterior(solu[x],next_sample[x])
