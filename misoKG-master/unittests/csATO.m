...
function [fn, FnVar, FnGrad, FnGradCov, constraint, ConstraintCov, ConstraintGrad, ConstraintGradCov] = csATO(x, runlength,
seed, arrivalratep1, arrivalratep2, arrivalratep3, arrivalratep4, arrivalratep5, ~)
% function [fn, FnVar, FnGrad, FnGradCov, constraint, ConstraintCov, ConstraintGrad, ConstraintGradCov] = csATO(x, runlength, seed, other);
...
% arrivalratep1, ..., arrivalratep5 arrival rates of the Poisson processes for products
% other is not used
...
if (sum(x < 0)>0 ) || (runlength <= 0) || (runlength ~= round(runlength)) || (seed <= 0) || (round(seed) ~= seed),
    ...
    products = [   arrivalratep1,    1,  0,  0,  1,  0,  1,  1,  0;
        arrivalratep2,      1,  0,  0,  0,  1,  1,  1,  0;
        arrivalratep3,    0,  1,  0,  1,  0,  1,  0,  0;
        arrivalratep4,    0,  0,  1,  1,  0,  1,  0,  1;
        arrivalratep5,    0,  0,  1,  0,  1,  1,  1,  0];
