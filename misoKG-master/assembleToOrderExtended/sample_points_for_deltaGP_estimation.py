...
if __name__ == "__main__":
    ...
    list_IS_to_query = [1,2] # [0]# for coldstart # range(obj_func_min._num_IS)
    ...
    # num_replications = 3
    num_init_pts_each_IS = 200 ###5 # for Rosenbrock # 20 # for ATO
    num_replications = 1
    ...
    for replication_no in range(num_replications):
        ...
        def parallel_func(IS, pt):
            return obj_func_min.evaluate(IS, pt) - obj_func_min.evaluate(0, pt)

        num_parallel_jobs = num_init_pts_each_IS    # Jialei's original choice
        if(('ato' in obj_func_min.getFuncName()) and (num_parallel_jobs > 16)): # do not start too many MATLAB instances
