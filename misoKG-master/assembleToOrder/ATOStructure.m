function [minmax d m VarNature VarBds FnGradAvail NumConstraintGradAvail StartingSol budget ObjBd OptimalSol] = ATOStructure(NumStartingSol, seed)
%function [minmax d m VarNature VarBds FnGradAvail NumConstraintGradAvail StartingSol budget ObjBd OptimalSol] = ATOStructure(NumStartingSol, seed);
% Inputs:
...
d = nItems; % select target inventory for each item
m = 0; % no constraints
VarNature = zeros(d, 1); % integer variables
VarBds = ones(d, 1)* [0, capacity]; % must buy a nonnegative quantity
FnGradAvail = 0; % No Derivative
NumConstraintGradAvail = 0; % No constraints
budget = [100; 3000; 9000; 15000]; %alter budget as needed
ObjBd = NaN;
OptimalSol = NaN; 
...
if (NumStartingSol < 0) || (NumStartingSol ~= round(NumStartingSol)) || (seed <= 0) || (round(seed) ~= seed),
    ...
    elseif (NumStartingSol == 1),
        StartingSol = capacity*ones(1,nItems);
    else
        ...
        StartingSol = unidrnd(capacity+1,NumStartingSol,nItems)-1; % Each row has each component uniformly distributed [0, capactiy]
