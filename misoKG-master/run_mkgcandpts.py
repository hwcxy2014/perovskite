...
'''
misoKG that we used for the NIPS submission

it samples better candidate pts to start BFGS
'''
...
list_mu_star_truth = []
list_raw_voi = []
init_best_idx = numpy.argmax(problem.hist_data._points_sampled_value[problem.hist_data._points_sampled[:, 0] == 0])
...
for kg_iteration in range(problem.num_iterations):
    ...
    with Parallel(n_jobs=num_threads) as parallel:
        for IS in problem.list_sample_is:
            ...
            # Select starting points for BFGS based on the KG value
            start_points_prepare = problem.obj_func_min.get_moe_domain().generate_uniform_random_points_in_domain(num_candidate_start_points)
            kg_vals = parallel(delayed(compute_kg_unit)(pt, IS) for pt in start_points_prepare)
            sorted_idx_kg = numpy.argsort(kg_vals)
            start_points = start_points_prepare[sorted_idx_kg[-num_multistart:], :]

            parallel_results = parallel(delayed(min_kg_unit)(pt, IS) for pt in start_points) #problem.obj_func_min.get_moe_domain().generate_uniform_random_points_in_domain(num_multistart))
        ...
        # Same pre-selection of points for find_mu_star()
        start_points_prepare = problem.obj_func_min.get_moe_domain().generate_uniform_random_points_in_domain(num_candidate_start_points)
        ...
        parallel_results = parallel(delayed(find_mu_star)(pt) for pt in start_points) #problem.obj_func_min.get_moe_domain().generate_uniform_random_points_in_domain(num_multistart))
    ...
    print "step {0}, best_truth {1}, sample_is {2}".format(kg_iteration, truth_at_best_sampled, sample_is)
