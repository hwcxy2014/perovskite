...
class solubilities(object):
	def __init__(self, n):
		# number of compounds, in this project n = 135
		self._dim = n
		# mean of solubility
		self._mu = np.zeros(n)
	...
	def setPrior(self, Z, mu_alpha,sig_alpha,mu_zeta,\
		...
		'''
		Z = np.array(Z)
		self._mu = [2*mu_alpha + mu_zeta for x in range(self._dim)]
		matern = hyper_parameters.maternKernel(Z[:,6:8],l1,l2,sig_m)
		for x in range(self._dim):
			for xx in range(self._dim):
				if x == xx:
					self._Sig[x,xx] = 2 * sig_alpha + sig_beta \
